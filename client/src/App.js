...
const schema = z.object({
  ...
  age: z.string().min(1).refine((val) => Number(val) < 120, { message: "Unrealistic Age" }),
  gender: z.string(),
  contact: z
    .string()
    .length(10, { message: "Phone number must be exactly 10 digits" })
    .refine((val) => val.split("").every((char) => char >= "0" && char <= "9"), {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-violet-100 to-violet-300 p-4">
      <div className="bg-white shadow-lg rounded-md w-full max-w-md p-10 ">
        <h2 className="text-2xl font-bold text-violet-600 text-center mb-6">Patient Profile</h2>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            <input
              ...
              className="w-full px-4 py-2 text-zinc-600 border rounded-sm focus:ring-1 focus:ring-violet-500 focus:outline-none"
            />
            {errors.contact && <div className="text-red-500 text-sm">{errors.contact.message}</div>}
          </div>
